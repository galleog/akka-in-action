...
class DataEnricher private constructor(context: ActorContext<Command>) :
    ...
    private fun onMessage(message: Message): Behavior<Command> {
        router.tell(Aggregator.Enriched(message.id, message.content + ":metadata"))
        return this
    }

    companion object {
        fun create(): Behavior<Command> = Behaviors.setup(::DataEnricher)
    }
}